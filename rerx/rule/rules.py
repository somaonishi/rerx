...
class RuleSet:
    ...
    def __init__(
        ...
        condition_map: Dict[int, "Condition"] = None,
        ...
        if ruleset is None:
            ...
            self.rules: List[Rule] = ruleset.rules
    ...
    def ordered_condition_map(self):
        if self.condition_map is not None:
            return {k: v for k, v in sorted(self.condition_map.items(), key=lambda item: str(item[1]))}

    def ordered_by_rule_condition_map(self):
        ordered_cond_map = dict()
        if self.condition_map is not None:
            for rule in self.rules:
                ordered_cond_map.update({k: v for k, v in sorted(rule.A, key=lambda item: str(item[1]))})
        return ordered_cond_map

    def set_categories(self, categories_dict: Dict[str, List[Any]]):
        for rule in self.rules:
            for cond_tuple in rule.A:
                cond = cond_tuple[1]
                att_name = cond.att_name
                value = cond.value
                if att_name in categories_dict.keys():
                    cond.set_category(categories_dict[att_name][value])
    ...
    def __str__(self):
        ...
        for rule in self.rules:
            ...
            rule_string = rule_string + " ˄ ".join(map(lambda cond: str(cond[1]), rule.A))
    ...
    def _predict(self, X, proba=False):
        ...
        if proba:
            ...
            prediction = np.zeros((X.shape[0], self.y_shape), dtype=self.classes[0].dtype)
    ...
    def print_rules(self, return_object=None, heuristics_digits=4, condition_digits=3):
        ...
        for rule in self:
            ...
            if len(rule.A) == 0:
                ...
                A_string = " ˄ ".join(sorted([cond[1].__str__(digits=condition_digits) for cond in rule.A]))
...
class Rule:
    ...
    def __init__(
        self,
        conditions: List[Tuple[int, "Condition"]],
        class_dist=None,
        ens_class_dist=None,
    ...
    def predict(self, X, proba=False):
        ...
        if len(self.A) > 0:
            mask = reduce(operator.and_, [cond[1].satisfies_array(X) for cond in self.A])
        if proba:
    ...
    def get_condition_map(self):
        return {cond[0]: cond[1] for cond in self.A}

    def add_conditions(self, rule: "Rule"):
        for cond in rule.A:
    ...
    def get_attrs_in_rule(self):
        return set([cond[1].att_name for cond in self.A])
...
class Condition:
    ...
    def __init__(self, att_index, op, value, att_name=None):
        ...
        if isinstance(value, float):
            self.str = f"({self.att_name} {op_dict[self.op.__name__]} " f"{self.value:.{3}f})"
        else:
            self.str = f"({self.att_name} {op_dict[self.op.__name__]} " f"{self.value})"
    ...
    def __str__(self, digits=3):
        ...
        return f"({self.att_name} {op_dict[self.op.__name__]} {value_str})"
